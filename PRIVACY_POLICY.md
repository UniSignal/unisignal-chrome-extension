...
- 最近 100 条频道消息及其频道 ID、消息 ID 保存在 Chrome 本地存储中，较旧消息会被自动替换，并保留至用户清除扩展数据或卸载扩展。
...
- The 100 most recent channel messages and their channel IDs and message IDs are stored in Chrome local storage. Older messages are automatically replaced, and the stored messages remain until the user clears Extension data or uninstalls the Extension.
